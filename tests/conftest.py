...
def collect_failures(results: list, predicate) -> list[dict]:
    return [
        {"index": i, "case": c, "result": r} for i, (c, r) in enumerate(results) if predicate(r)
    ]


def save_failures(failures: dict, test_name: str, timestamp: "str | None" = None) -> "Path | None":
    if not failures:
        return None
...
def run_test(test_name: str, results: list, predicate, description: str) -> list:
    ...
    if failures:
        save_failures({"ordinary": failures}, test_name, datetime.now().strftime("%Y%m%d_%H%M%S"))
    if failures:
