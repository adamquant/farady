    ...
    if asib == None:
        ...
        # FARD AL-UKHT LI-AB
            ...
            if (
                ...
                and not any([shaqiqas_taking_twothirds, bints_taking_twothirds, bint_taking_half])
            ):  # takmula
...
def calculate(new):
    ...
    is_umuriya1 = all(
        [new.get("ab"), new.get("umm"), new.get("zawj"), not any([has_jame, has_any_furoo])]
    )
    is_umuriya2 = all(
        [new.get("ab"), new.get("umm"), new.get("zawja"), not any([has_jame, has_any_furoo])]
    )
    ...
    if is_umuriya:
        ...
        if has_any_furoo:
            finish, asib, bint_taking_half, bints_taking_twothirds = furoo(
                new, finish, asib, has_hawashi
            )

        if has_hawashi:
            if not any([has_m_usool, has_m_furoo]):
                finish, asib, shaqiqa_taking_half, shaqiqas_taking_twothirds = hawashi(
                    new,
                    finish,
                    asib,
                    bint_taking_half,
