...
def hawashi_step(case: Case) -> Case:
    ...
    if case.shaqiq.get("count", 0) and not case.shaqiqa.get("count", 0):
        ...
        not case.shaqiq.get("count", 0) and case.shaqiqa.get("count", 0) and not case.has_any_furoo
    ):
...
def taseeb_step(case: Case) -> Case:
    ...
    if case.asib == "ibn":
        ...
        case.ibn["shares"] = case.ibn.get("shares", 0) + case.ibn.get("count", 0) * 2 * taseeb_unit
        case.bint["shares"] = case.bint.get("shares", 0) + case.bint.get("count", 0) * taseeb_unit
    elif case.asib == "iibn-bibn":
        case.iibn["shares"] = (
            case.iibn.get("shares", 0) + case.iibn.get("count", 0) * 2 * taseeb_unit
        )
        case.bibn["shares"] = case.bibn.get("shares", 0) + case.bibn.get("count", 0) * taseeb_unit
    elif case.asib == "iiibn-biibn":
        case.iiibn["shares"] = (
...
def awl_step(case: Case) -> Case:
    ...
    """
    total_fard = sum(heir.get("fard", frac(0)) for heir in case._all_heirs if heir.get("fard"))
...
def create_mini_case_for_spouses(case):
    mini_case_spouse = Case(
        ...
        zawja=(
            case.zawja.copy() if (case.zawja.get("count", 0) or case.zawja.get("shares", 0)) else {}
        ),
    )
